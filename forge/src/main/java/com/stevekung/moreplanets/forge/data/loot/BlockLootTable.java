...
import com.stevekung.stevekunglib.forge.data.loot.BlockLootBase;
...
public class BlockLootTable extends BlockLootBase
{
    ...
    protected void addTables()
    {
        ...
        this.add(MPBlocks.PURLONITE_CLUSTER, block -> createSilkTouchDispatchTable(block, LootItem.lootTableItem(MPItems.PURLONITE_SHARD).apply(SetItemCountFunction.setCount(ConstantValue.exactly(4))).apply(ApplyBonusCount.addOreBonusCount(Enchantments.BLOCK_FORTUNE)).when(MatchTool.toolMatches(ItemPredicate.Builder.item().of(MPItems.GLOWING_IRON_PICKAXE))).otherwise(applyExplosionDecay(block, LootItem.lootTableItem(MPItems.PURLONITE_SHARD).apply(SetItemCountFunction.setCount(ConstantValue.exactly(2)))))));
