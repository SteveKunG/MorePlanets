...
public class EntityVeinEye extends Entity
{
    ...
    public void onUpdate()
    {
        ...
        {
            this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, this.posX - this.motionX * f3 + this.rand.nextDouble() * 0.6D - 0.3D, this.posY - this.motionY * f3 - 0.5D, this.posZ - this.motionZ * f3 + this.rand.nextDouble() * 0.6D - 0.3D, this.motionX, this.motionY, this.motionZ, new int[] { Block.getIdFromBlock(NibiruBlocks.INFECTED_VINES) });
        }
        ...
        if (!this.worldObj.isRemote)
        {
            ...
            if (this.despawnTimer > 80 && !this.worldObj.isRemote)
            {
                ...
                {
                    this.worldObj.spawnEntityInWorld(new EntityItem(this.worldObj, this.posX, this.posY, this.posZ, new ItemStack(NibiruItems.VEIN_EYE)));
                }
                else
                {
                    ...
                    for (int j = 0; j < 8; ++j)
                    {
                        this.worldObj.spawnParticle(EnumParticleTypes.ITEM_CRACK, d0, d1, d2, this.rand.nextGaussian() * 0.15D, this.rand.nextDouble() * 0.2D, this.rand.nextGaussian() * 0.15D, new int[] {Item.getIdFromItem(NibiruItems.VEIN_EYE)});
                    }
                    for (double d18 = 0.0D; d18 < Math.PI * 2D; d18 += 0.15707963267948966D)
                    {
                        this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, d0 + Math.cos(d18) * 5.0D, d1 - 0.4D, d2 + Math.sin(d18) * 5.0D, Math.cos(d18) * -5.0D, 0.0D, Math.sin(d18) * -5.0D, new int[] { Block.getIdFromBlock(NibiruBlocks.INFECTED_VINES) });
                        this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, d0 + Math.cos(d18) * 5.0D, d1 - 0.4D, d2 + Math.sin(d18) * 5.0D, Math.cos(d18) * -7.0D, 0.0D, Math.sin(d18) * -7.0D, new int[] { Block.getIdFromBlock(NibiruBlocks.INFECTED_VINES) });
                    }
                }
            }
        }
