...
public class BlockDarkEnergyReceiver extends BlockTileMP implements IBlockDescription, ISingleBlockRender
{
                        ...
                        if (!tile.disabled)
                        {
                            if (tile.getEnergyStoredGC() > 20000.0F)
                            {
                                if (!tile.isActivated())
                                {
                                    ...
                                    player.addChatMessage(new JsonUtil().text(GCCoreUtil.translate("gui.dark_energy_success.message")).setStyle(new JsonUtil().colorFromConfig("green")));
                                    return true;
                                }
                                else
                                {
                                    player.addChatMessage(new JsonUtil().text(GCCoreUtil.translate("gui.dark_energy_already_active.message")).setStyle(new JsonUtil().red()));
                                    return false;
                                }
                            }
                            else
                            {
                                if (!tile.isActivated())
                                {
                                    player.addChatMessage(new JsonUtil().text(GCCoreUtil.translate("gui.dark_energy_no_power.message")).setStyle(new JsonUtil().red()));
                                    return false;
                                }
                            }
                        }
            ...
            else
            {
                if (world.getTileEntity(pos) instanceof TileEntityDarkEnergyReceiver)
                {
                    ...
                    if (tile.failedTick > 0 && !tile.successful)
                    {
                        ...
                        player.addChatMessage(new JsonUtil().text(GCCoreUtil.translateWithFormat("gui.dark_energy_malfunction.message", this.ticksToElapsedTime(failedTick)) + " second" + s + "!").setStyle(new JsonUtil().red()));
                        return false;
                    }
                    else
                    {
    ...
    private boolean checkBlock(World world, BlockPos pos, EntityPlayer player, IBlockState state, boolean flag)
    {
        ...
        else
        {
            if (!flag)
            {
                player.addChatMessage(new JsonUtil().text("Missing block " + state.getBlock().getLocalizedName() + " at " + pos.getX() + " " + pos.getY() + " " + pos.getZ()).setStyle(new JsonUtil().red()));
            }
            return false;
        }
    }
