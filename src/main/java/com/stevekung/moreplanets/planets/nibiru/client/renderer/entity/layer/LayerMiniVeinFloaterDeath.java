...
{
    @Override
    public void doRenderLayer(EntityMiniVeinFloater entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale)
    {
        if (entity.deathTicks > 0)
        {
            ...
            for (int i = 0; i < (f + f * f) / 2.0F * 60.0F; ++i)
            {
                ...
                worldrenderer.pos(0.866D * f3, f2, -0.5F * f3).color(110, 60, 10, 0).endVertex();
                worldrenderer.pos(0.0D, f2, f3).color(110, 60, 10, 0).endVertex();
                worldrenderer.pos(-0.866D * f3, f2, -0.5F * f3).color(110, 60, 10, 0).endVertex();
                tessellator.draw();
            }
