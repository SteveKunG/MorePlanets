package stevekung.mods.moreplanets.core.world.temp;
...
public class ChunkProviderJupiter extends ChunkProviderGenerate
{
    ...
    public ChunkProviderJupiter(World par1World, long par2, boolean par4)
    {
    ...
    public void generateTerrain(int chunkX, int chunkZ, Block[] idArray, byte[] metaArray)
    {
        ...
        for (int x = 0; x < ChunkProviderJupiter.CHUNK_SIZE_X; x++)
        {
            for (int z = 0; z < ChunkProviderJupiter.CHUNK_SIZE_Z; z++)
            {
                ...
                for (int y = 0; y < ChunkProviderJupiter.CHUNK_SIZE_Y; y++)
                {
                    if (y < ChunkProviderJupiter.MID_HEIGHT + yDev)
                    {
        ...
        for (int cx = chunkX - 2; cx <= chunkX + 2; cx++)
        {
            for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++)
            {
                for (int x = 0; x < ChunkProviderJupiter.CHUNK_SIZE_X; x++)
                {
                    for (int z = 0; z < ChunkProviderJupiter.CHUNK_SIZE_Z; z++)
                    {
                        if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * ChunkProviderJupiter.CHUNK_SIZE_X + x, cz * ChunkProviderJupiter.CHUNK_SIZE_Z + z) / ChunkProviderJupiter.CRATER_PROB)
                        {
